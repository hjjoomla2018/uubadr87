	...
	<?php 
			...
			if(count($modules_djcf)>0){
				echo '<div class="djcf-ad-items-top clearfix">';
					...
					$position   = 'djcf-items-top';
					$options   = array('style' => 'jmmodule');
									...
									if($c!=null){
										echo '<div class="title">';
											echo '<h2>';
												...
												echo '<a href="'.DJClassifiedsSEO::getCategoryRoute($c->id.':'.$c->alias).$anch.'">';											
													echo '<span><img src="'.JURI::base(true).$this->cat_images[$c->id]->path.$this->cat_images[$c->id]->name.'_ths.'.$this->cat_images[$c->id]->ext.'" alt="'.$c->name.'" /></span>';
													}
												if($par->get('show_adsn_cat')){
														if(!$c->items_count){
															$c->items_count=0;
														}
														echo '<span>'.$c->name.' <span>('.$c->items_count.')</span></span>';												
													}else{
